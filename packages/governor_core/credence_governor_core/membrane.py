...
def handshake_decl() -> dict[str, Any]:
    ...
    default utility table, echo all-false (the epoch-1 restriction)."""
...
class MembraneSession:
    ...
    def boot(self) -> None:
        reply = self.client.request(handshake_decl())
        if not reply.get("ok"):
        ...
        # the local observation log on top, in arrival order (8.2)
    ...
    def decide(
        ...
        deliberately unused (HOSTS_PLAN 2.1)."""
        ...
        reply = self._tick({
            "features": feats,
            "menu": MENU_IDS,
            "utility": {"form": "table@1", "rows": utility_rows(profile)},
        })
    ...
    def observe(self, features: dict[str, str], observation: int) -> None:
        ...
        re-sent with the verdict; the evidence clock advances."""
        feats = onehot(features)
        feats["t"] = float(self._t)
        self._tick({"features": feats, "evidence": int(observation)})
        self._t += 1
...
def build_membrane_session(
    ...
    return MembraneSession(MembraneClient.from_env(logline), brain_dir, log, logline)
